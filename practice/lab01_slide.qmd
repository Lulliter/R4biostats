...
author: "<a href='https://r4biostats.com/teacher.html' style='color:#72aed8;font-weight:600;'>M. Chiara Mimmi, Ph.D.</a>&ensp;|&ensp;Università degli Studi di Pavia"
date: 2024-07-25
date-format: long
code-link: true
format:
...
execute:
  echo: true
  eval: true # set to false in specific code chunks
  output: true
  freeze: auto
---
...
Instead, the fantastic [`here`](https://here.r-lib.org/) package lets you reference file paths in a *reproducible* manner (anchored on the R Project's folder as the *root*). 
...
Create a Sub-Directory (e.g. for saving input data and output data)
...
## (optional) Clean up my workspace^[Warning: you will not have these objects in your workspace fo rlater use]
...
-   using {janitor} function `tabyl` {janitor} uses the "pipe" operator `%>%` which takes the output of a function as input of the next one
```{r}
...
```{r}
head(autism_pids, n = 2) #return fist 3 obs
tail(autism_pids, n = 2) #return last 3 obs
```
...
I want to ID those patients with missing `age`
...
⚠︎ WARNING ⚠︎ This is a very delicate steon, because any data that is modified or imputed beyond the original collection can affect the result of subsequent analysis and statistical modeling.
...
## Saving the one plot and a RDS data file.
