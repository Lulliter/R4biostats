...
# ML: EXAMPLE of UNSUPERVISED ALGORITHM
...
## [Principal Component `Scores` plot]{.r-fit-text}
...
1. We need to retrieve the principal component scores by calling `biopsy_pca$x`, and  store them in a data set called `PC_scores`.
...
3. In addition, if the data includes a factor variable, like in this case, it may be interesting to show the grouping on the plot as well.
...
The Scores Plot can be visualized via the ggplot2 package. The grouping is indicated by the color argument (Label); the geom_point() is used to plot the point cloud.
...
::: {style="font-size: 80%;"}
A 3D scatterplot of observations shows the first three principal components’ scores. 

+ We use the `scatterplot3d()` function of the `scatterplot3d` package, with the color argument assigned to the Label variable
...
Next, we use another special type of scatterplot (a **biplot**) to understand the relationship between the principal components and the original variables. 

+ For this we have the `fviz_pca_biplot()` function from the `factoextra` package 
...
::: {style="font-size: 95%;"}
Biplots have two components: **scores** and **loading vectors**. 
As in the scores plot each point represents a sample in the space of principal components.

+ Biopsies of the same class are located closer to each other, which indicates that they have similar principal component scores. 

+ The loading vectors represent the original variables in the space of principal components. As expected from PCA, the single `PC1` accounts for variance in almost all original variables, while `V9` has the major projection along `PC2`.

+ The loading vectors represent strength and direction of association of original variables with new PC variables.
...
::: {.r-fit-text}
> *"OK, but how big of a sample do I need?* 
...
# Select a portion of the (pilot) data
...
## [Two Paired Samples t-test: EXE data]{.r-fit-text}
::: {style="font-size: 85%;"}
...
## [Two Paired Samples t-test: EXAMPLE cont.]{.r-fit-text}
...
## [Two Paired Samples t-test: POWER ANALYSIS (`n`)]{.r-fit-text}
...
> As a matter of fact, would have only needed ` n = 8` pairs of observations in this study, given the size of effect we were trying to detect.
...
> The RMSE (= 6.579572) tells us, (roughly speaking) by how much, on average, the new observed BMI values differ from those predicted by our model
:::


## [... and what about in trainig data?]{.r-fit-text}
...
summary(lin_mod)$r.squared     # R^2 0.03374094
```
