...
With the package `devtools` and its function `install_github`^[After `devtools::install_github("MilesMcBain/paint")`, R asks me if I want to update related packages... respond in the console choosing the preferred answer.] to install the developer's version of a package. Let's try it with a little package `paint` (which colors the structure of dataset when printing).
...
For example, create a sub-directory with `fs` package (for saving input data and output data)
...
### char to factor (n cols) using `base`
...
I may prefer to code a variable as logical. Let's recode  `age_desc`
```{r}
...
I also may need my binary variables expressed as [0,1] (e.g. to incorporate nominal variables into regression analysis). Let's recode `autism`.
...
⚠︎ WARNING︎: This is a very delicate step⚠︎

+ any data that are modified or imputed beyond the *original collection* can affect the result of subsequent analysis and statistical modeling 
...
For uneven $n$: $Mdn = \frac{x_{(n+1)}}2$ For even $n$: $Mdn = \frac{x_{(n/2)} + x_{(n/2+1)}}2$
...
Recall that: <br>
**Population Variance** $\sigma^2 = \frac{\displaystyle\sum_{i=1}^{n}(x_i - \mu)^2} {n}$ <br><br>
**Sample Variance** $s^2 =\frac{\sum{(x_i-\bar{x})^2}}{n-1}$  
<br><br>
**Population Standard deviation** $\sigma = \sqrt\frac{\displaystyle\sum_{i=1}^{n}(x_i - \mu)^2} {n}$ <br><br>
...
First, I need to reformat `group_stats`, changing it from a "wide" format with each variable in its own column to a "long" format, where you use one column for all the *measures names* and another for all the *measures values*.

+ This can be done using `tidyr::pivot_longer` function
...
+ (which we can do because ethnicity is coded as factor)
