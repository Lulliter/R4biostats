...
# Day 1: topics illustrated in class
...
Let's take for example the R package `corrplot`, a package for graphically exploring correlations. To install it for the first time, open an R session and execute:
...
So if you source a file like this:
```{r}
#| eval: false
#| echo: true
...
This is where the fantastic [`here`](https://here.r-lib.org/) package comes in, as it will define file paths in a "reproducible manner" (anchored on the R Project). 

1. It let's you use **relative paths**, i.e. specify the file path relative to the project folder. 
...
### Example of problem | solution tab

::: {#exercise-autism .callout-tip}
## Loading input data in your workspace

::: panel-tabset
## Problem

1.  Load the dataset `autism_data.csv` in your workspace with code that won't break on another machine / OS

## Hint

Package `here` will help 
```{r}
#| label: set-up
#| echo: true
